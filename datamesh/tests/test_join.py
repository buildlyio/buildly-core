...
def test_join_data_one_obj_w_relationships(
    mock_perform_request, mock_spec, auth_api_client, relationship
):
    ...
    # mock first response
    service_response = (
        {'id': 1, 'name': 'test', 'contact_uuid': 1},
        200,
        {'Content-Type': ['application/json']},
    )
    # mock second response
    expand_response = (
        {'id': 1, 'file': '/somewhere/128/'},
        200,
        {'Content-Type': ['application/json']},
    )
...
def test_join_data_one_obj_w_two_relationships(
    mock_perform_request, mock_spec, auth_api_client, relationship, relationship2
):
    factories.JoinRecord(
        relationship=relationship,
        record_id=1,
...
@patch('gateway.request.GatewayRequest._get_swagger_spec')
@patch('gateway.request.SwaggerClient.request')
def test_join_data_list(
    mock_perform_request, mock_spec, auth_api_client, relationship_with_10_records, org
):
    mock_spec.return_value = Mock(Spec)

    join_records = relationship_with_10_records.joinrecords.all()
    main_service_data = [
        {'uuid': item.record_uuid, 'name': f'Boiler #{i}'}
        for i, item in enumerate(join_records)
    ]
    main_service_response = (
        main_service_data,
        200,
        {'Content-Type': ['application/json']},
    )
    ...
    # make api request
    path = '/{}/{}/'.format(
        relationship_with_10_records.origin_model.logic_module_endpoint_name, 'products'
    )
    response = auth_api_client.get(path, {'join': ''})
